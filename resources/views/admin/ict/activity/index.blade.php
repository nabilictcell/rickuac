      ...
      <section class="panel">
          <header class="panel-heading">
           ...
           <div class="btn-group pull-right">
	             <a href="{{ route('ict-activity-create')}}" class="btn btn-md btn-success">Add New</a>
	       </div>
          </header>
          ...
          <div class="panel-body">
            <div class="adv-table">
                ...
                <tbody>
                    ...
                    <?php
                     $category = DB::table('new_activity_categories')->where('id',$item->category_id)->first();
                    ?>
                  <tr>
                      <td>{{$loop->index+1}}</td>
                      <td>{{ $item->title }}</td>
                      ...
                      <td>
                          <a href="{{ route('ict-activity-edit',['id'=>$item->id]) }}" class="btn btn-md btn-success">Edit</a>
                          ||
                          <form id="delete-form-{{ $item->id }}" method="post" action="{{ route('ict-activity-destroy',['id'=>$item->id]) }}" style="display: none">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                        </form>
