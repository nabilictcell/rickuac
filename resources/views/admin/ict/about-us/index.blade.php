      ...
      <section class="panel">
          <header class="panel-heading">
           ...
           <div class="btn-group pull-right">
	             <a href="{{ route('ict-about-us-create')}}" class="btn btn-md btn-success">Add New</a>
	       </div>
          </header>
            ...
            <div class="adv-table">
                ...
                <tbody>
                  ...
                  <tr>
                      ...
                      <td>{{ $item->title }}</td>
                      <td>{{ $item->category }}</td>
                      <td><a href="{{ $item->link }}" alt="" width="50" height="40">Click Here View</td>
                      <td>{{ $item->slug }}</td>
                      <td>
                          <a href="{{ route('ict-about-us-edit',['id'=>$item->id]) }}" class="btn btn-md btn-success">Edit</a>
                          ||
                          <form id="delete-form-{{ $item->id }}" method="post" action="{{ route('ict-about-us-destroy',['id'=>$item->id]) }}" style="display: none">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                        </form>
